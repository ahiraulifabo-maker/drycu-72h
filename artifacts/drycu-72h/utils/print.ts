  ...
  try {
    ...
    if (typeof document !== 'undefined') {
      ...
      inputs.forEach((inp: any) => {
        ...
        else if (val.length > 2 && isNaN(val) && !val.includes('@') && !['dry', 'clean', 'true', 'false'].some(w => val.toLowerCase().includes(w))) {
          if (customerName === 'Customer' || /^[0-9a-zA-Z_-]{15,}$/.test(customerName)) {
            customerName = val;
          }
        }
      });
