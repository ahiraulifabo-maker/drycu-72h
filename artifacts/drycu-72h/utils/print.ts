...
// ==========================================
// 2. MAIN INVOICE/BILL PRINTING LOGIC
// ==========================================
export function printBill(order: any, storeInfo: any) {
  ...
  try {
    // Abhi ke liye crash rokne ke liye ek temporary standard receipt layout
    let itemRows = '';
    const items = order.items || [];
    let totalAmount = 0;
    ...
    items.forEach((item: any) => {
      ...
      itemRows += `
        <tr>
          ...
          <td style="text-align: right; padding: 5px 0;">₹${itemTotal}</td>
        </tr>
      `;
    });
    ...
    const html = `
      ...
      <head>
        ...
        <style>
          ...
          body { font-family: monospace; width: 58mm; margin: 0; padding: 10px; font-size: 12px; }
      ...
      <body>
        ...
        <div class="center">Mob: ${storeInfo.phone || ''}</div>
        <div class="border-top" style="margin-top: 5px;"></div>
        <p><b>Order ID:</b> ${order.id || ''}<br><b>Date:</b> ${new Date().toLocaleDateString()}</p>
        ...
        <div class="border-top" style="margin-top: 5px;"></div>
        <p class="bold" style="text-align: right; font-size: 14px;">Total: ₹${totalAmount}</p>
        <div class="center" style="margin-top: 15px;">Thank You! Visit Again.</div>
      </body>
      </html>
    `;
